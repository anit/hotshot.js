(function ($) {
  ...
  CGrid.prototype = {
    ...
    edit: function (targetRowIndex, obj) {
      var tableRow = this.listTable.find('.cgrid-data-container > tr')[parseInt(targetRowIndex)];
  ...
  var CGridHelpers = {
    ...
    create: function (cGrid, listTableTemplate) {
      var i = 0;
      var listTable = $(listTableTemplate);

      //create header
      var tableHeaderRow = listTable.find('.cgrid-header-row');
      if (cGrid.props.columns) {
         for (i in cGrid.props.columns) {
            var col = $('<td></td>');
            col.html(cGrid.props.columns[i].header);
    ...
    *
    */
    serializeForm: function (formContainer) {
        var object = {};
        formContainer.find('[name]').each(function () {
